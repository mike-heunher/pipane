...
Backends advertising `content-addressed-session-sync` accept an optional cached state hash and exact materialized-message hash inventory on `subscribe_session`. An equal state hash produces `not_modified`. Otherwise the backend sends an authoritative ordered message-hash manifest, the small non-message state, and only message bodies absent from the browser inventory. The browser reconstructs and verifies the canonical full-state hash before committing it; missing, corrupt, evicted, or incompatible cache entries fall back to a full snapshot. Serialized snapshots are kept in a byte- and count-bounded IndexedDB LRU, scoped by backend and session, while only the active session is parsed into memory. Subsequent live updates retain the existing hash-chained text deltas.

These shapes are additive within application v1: older browsers never request content-addressed initialization and therefore continue receiving full snapshots. Revisions keep ordering and recovery explicit without pre-empting the future semantic-update protocol.
...
An authorized browser may attach a bounded, validated list of temporary TURN credentials to `connect_backend`. Rendezvous merges these with centrally issued ICE servers and forwards them only in that route's existing `connection_request`; it does not persist them. Long-term Metered API keys and coturn REST shared secrets remain in browser-local IndexedDB, where the browser exchanges or derives fresh per-connection credentials. Static provider credentials are supported as an explicit advanced fallback. Diagnostics expose only configured ICE URLs, never usernames, credentials, API keys, or shared secrets.
...
- `files.read`, `files.upload.create`, `files.upload.append`, `files.upload.complete`, `files.upload.abort`
- `host.browse`, `host.mkdir`
- `settings.get`, `settings.validate`, `settings.patch`, `settings.save`
- `updates.get`, `updates.run`

Every method has runtime-validated parameters, correlated responses, stable error codes, bounded concurrency, and a bounded device-scoped completed-request cache. Relative `files.read` paths preserve their relative form across the carrier so the backend can resolve successful write/edit targets first, then a same-repository Git worktree evidenced by the active conversation, and finally the immutable session CWD; exact conversation mentions remain the fallback for other outside-CWD paths. The browser normalizes Pi-generated `sandbox:/absolute/path` links to that absolute backend path before `files.read`, matching its handling of local `file:` URLs. File uploads use bounded, offset-addressed base64 chunks so attachments can cross either local HTTP or the authenticated DataChannel and land in a private durable backend path. Upload manifests survive backend restarts, failed browser uploads are explicitly aborted, incomplete uploads expire after one hour, and completed uploads unreferenced by any session expire after a seven-day grace period. Non-image paths are referenced in the agent prompt. A backend advertising `uploaded-image-prompt` accepts the exact completed-upload path in the small application prompt frame, then validates and materializes the bytes onto Pi's direct image-content path; multi-megabyte image data is therefore never placed in one browser prompt frame. The browser retains the legacy inline image command for hosts that do not advertise this additive shape, so a public preview bundle remains compatible with older authorized backends. Pending browser requests retain their id across a carrier reconnect, so an in-flight mutation is resumed or answered from the cache instead of being executed twice. The backend uses one `LocalBackendApi` implementation for both the local HTTP facade and semantic DataChannel requests; `/api/capabilities` exposes the same capability document to a same-origin browser. Remote session results are scoped to a structured `{ backendId, path }` identity; paths remain backend-local identifiers rather than authorization.
