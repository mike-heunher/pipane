...
`subscribe_session` may include the SHA-256 `baseHash` of a browser-cached state. The backend retains a bounded patch journal and responds with one hash-chained delta (including an empty delta when the state is already current) whenever that baseline remains resumable. This first resumed delta establishes a new connection-local revision scope; an unknown, evicted, rewritten, or invalid baseline safely falls back to a full snapshot. Browser baselines are backend-scoped, memory-cached immediately, and coalesced into a bounded IndexedDB cache so reconnects, session switches, and page reloads do not normally retransmit conversation history. A hash failure deletes the cache entry and requests one unbased full state.
...
After the unfragmented authentication exchange, the carrier transparently gzip-compresses logical frames larger than 12,000 UTF-8 bytes when compression reduces their physical size, then splits the resulting bytes into ordered base64 chunk envelopes no larger than 16 KiB. The same transparent envelopes are used for large server-to-browser local WebSocket frames. Browser and backend validate the advertised gzip size and reassemble/decompress at most 64 MiB per logical frame with bounded pending-frame and outgoing-queue memory. Application v1 and semantic v2 decoders therefore continue to receive exactly one complete JSON frame regardless of compression or the negotiated SCTP message-size limit.

Server carriers schedule control and bulk lanes independently, so responses, status, steering, and abort traffic can overtake queued session-snapshot chunks. Selecting another session cancels the unsent portion of the stale snapshot with a reserved carrier envelope before the replacement begins. Detached JSONL watcher bursts are coalesced per session and attached streaming publication is capped at one update per 50 ms, matching useful browser render cadence. Both paths are hash-chained against each client's last state, produce a delta when it is smaller than a full snapshot, and suppress unchanged updates. Regaining tab visibility refreshes only compact statuses because reliable ordered carriers continue receiving while hidden.

Remote connection diagnostics retain exact Pipane logical-frame and physical-message byte counters across WebRTC reconnects for the lifetime of the page. They break received logical bytes down by protocol frame type, alongside browser WebRTC/SCTP statistics; physical payload counters intentionally exclude lower-level IP, ICE, DTLS, and retransmission overhead.
